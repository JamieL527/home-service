...
import { SendOfferButton } from '@/app/(admin)/admin/jobs/[id]/match/send-offer-button'
...
export default async function SalesContractorMatchPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  if ((job.status as string) !== 'READY') {
    return (
      <div className="animate-fadeIn max-w-2xl">
        <Link href="/sales/jobs" className="inline-flex items-center text-sm text-gray-400 hover:text-gray-700 font-semibold mb-5 transition-colors">
          ← Jobs
        </Link>
            ...
            return (
              <div key={company.id} className={`bg-white border rounded-2xl p-5 shadow-sm flex items-start gap-4 ${
                ...
                <div className="flex-1 min-w-0">
                  <div className="flex items-center gap-2 flex-wrap mb-1">
                    ...
                    {company.registrationType === 'referral' && (
                      <span className="text-[10px] font-bold bg-purple-50 text-purple-700 border border-purple-200 rounded-full px-2 py-0.5">Referral</span>
                    )}
                    {isDeclined && (
                      <span className="text-[10px] font-bold bg-red-50 text-red-500 border border-red-200 rounded-full px-2 py-0.5">Declined</span>
                    )}
                    ...
                    {!isPending && !isDeclined && s === 1 && (
                      <span className="text-[10px] font-bold bg-blue-50 text-blue-600 rounded-full px-2 py-0.5">Trade Match</span>
                    )}
                  </div>
                  <div className="flex flex-wrap gap-x-4 gap-y-0.5 text-xs text-gray-500 mt-1">
                    {company.tradeType && <span className="flex items-center gap-1"><Wrench size={10} className="text-gray-400" />{company.tradeType}</span>}
                    {company.address && <span className="flex items-center gap-1"><MapPin size={10} className="text-gray-400" />{company.address}</span>}
                    {company.contactName && <span className="font-medium text-gray-600">{company.contactName}</span>}
                ...
                <div className="shrink-0">
                  {isPending ? (
                    <span className="px-3 py-1.5 text-[11px] font-bold bg-gray-100 text-gray-400 rounded-lg">Offer Sent</span>
                  ) : (
